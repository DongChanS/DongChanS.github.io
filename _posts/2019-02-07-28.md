...
그러나 ORM으로 만들어놓으면 app.config["SQLALCHEMY_DATABASE_URL"] = "sqlite:///blog2.db"

이 문장을 postgre에 대해서 바꿔놓으면 **ORM이 모두 postgre db의 문법으로 맵핑**시켜주기 때문에 매우 편리하다.
...
이러한 상황을 막기 위해서 은행 DBMS에는 **transection**이라는 개념이 있다.

(transection : 여러 가지 sql문장들을 쌓아서 한번에 실제로 반영시키는 하나의 단위)

A와 B는 각자의 transection을 가지고 있으며,

A가 (Commit을 통해서) 인출명령을 내리면 A의 transection이 시행되며, 동시에 다른 transection들의 접근을 막는다 (**transection locking**) 그러면 그 시간에는 B의 transection이 시행될 수 없는 것이다.
...
그래서 Session을 커밋하면 이제껏 구성한 transection이 시행되는 개념이라고 생각하면 된다.
