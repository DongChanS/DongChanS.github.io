...
### 2-2. [**Huffman coding trees**](https://en.wikipedia.org/wiki/Huffman_coding) thar are used in data compression algorithms.
...
함수는 매우 정직하기 때문에 **먼저 만나는 함수가 끝날 때 까지 다음 함수를 실행하지 않습니다.** 이러한 방식은 **Stack의 FIFO**(First In First Out) 방식과 매우 비슷하며, 재귀함수를 스택으로 구현하기도 매우 쉽습니다.
